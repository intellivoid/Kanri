...
from haruka import app, plate, tmp_lang, httpsession
...
class ETagCacheManager:
	...
	async def get(self, url):
		...
		if not url in self.etagcache:
			...
			data = await httpsession.get("https://raw.githubusercontent.com/Evolution-X-Devices/official_devices/master/devices.json", headers=headers)
			if data.status == 304:
...
@app.on_message(~filters.me & filters.command('los'))
...
@app.on_message(~filters.me & custom_filters.command('evo'))
...
@app.on_message(~filters.me & custom_filters.command('phh'))
...
@app.on_message(~filters.me & custom_filters.command('bootleggers'))
	...
	try:
		...
		for codename, info in data.items():
			if cmd == codename:
				...
				buttons = InlineKeyboardMarkup(
						[[InlineKeyboardButton(text=plate("android_button_download", tmp_lang), url=info['download'])]])
				await message.reply(plate("android_bootleg_msgtxt", tmp_lang,
						name=info['fullname'],
						maintainer=info['maintainer'],
						date=info['buildate'],
						size=sizee(int(info['buildsize'])),
...
@app.on_message(~filters.me & custom_filters.command('magisk'))
async def magisk(client, message):
	url = 'https://raw.githubusercontent.com/topjohnwu/magisk_files/'
	try:
		stable, beta, canary = await asyncio.gather(cache.get(url + "master/stable.json"), cache.get(url + "master/beta.json"), cache.get(url + "canary/canary.json"))
		if not stable and not beta and not canary:
			await message.reply(plate("android_err_api", tmp_lang))
			return

		await message.reply(plate("android_magisk_msgtxt", tmp_lang,
				# Generics
